...
- Task scheduling – Celery and Celery Beat run tasks on a schedule defined by configuration (for YAML-driven runs, by each group’s `default_time`). The **boost_collector_runner** app builds the Beat schedule from `config/boost_collector_schedule.yaml` when the YAML loads successfully; if the YAML is missing or invalid, `CELERY_BEAT_SCHEDULE` is set to `{}` (no autogenerated schedule)—there is no fallback beyond what you configure. Redis is the message broker. Run by hand: `python manage.py run_scheduled_collectors --schedule daily` (and optional `--group`). Start the worker with `celery -A config worker -l info` and the scheduler with `celery -A config beat -l info`.

...
from cppa_slack_tracker.api_schemas import parse_channel, parse_team
...
def sync_team(team_id: str, team_name: Optional[str] = None) -> SlackTeam:
    ...
    if not name or name == team_id:
        ...
        if team_data:
            ...
            fetched_id = team_data.team_id or team_id
            if fetched_id == team_id:
                name = (team_data.team_name or "").strip() or team_id
            else:
...
def _process_channel_info(ch, team: SlackTeam) -> bool:
    """
